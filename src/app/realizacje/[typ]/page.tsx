...
import {ListRealizacjeData} from "@firebasegen/default-connector";
import {Realizacja, RealizacjeArray} from "@/utils/test";

export default async function Page({params}: {
    params: Promise<{ typ: string }>
}) {
    const typ = decodeURI((await params).typ);
    const { realizacjas }: ListRealizacjeData = await getListRealizacje();
    const realizacja: Realizacja = realizacjas.filter(value => value.typ === typ)[0];

    return (
        <>
            <ContainerWeb idStr={''} className={'bg-orangeDark'}>
                <div className={'flex flex-row justify-start'}>
                    <Topic name={realizacja.typ}/>
                </div>
                <Suspense fallback={<KontenerKartLoadin realizacjeData={realizacjas}/>}>
                    <KontenerKart realizacja={realizacja}/>
                </Suspense>
            </ContainerWeb>
...
async function KontenerKart({realizacja}: { realizacja: Realizacja }) {
    const realizacje = await getListOfFolder('/realizacje/' + realizacja.typ);
    const realizacje2 = (
        await Promise.all(
            ...
            <div className={`grid grid-cols-fill-300  lg:grid-cols-fill-300 gap-x-5 gap-y-16 justify-around`}>
                {realizacje2.map((value, index) =>
                    <Suspense fallback={KartaLoading()} key={value}>
                        <Karta realizacja={realizacja} numer={index + 1}/>
                    </Suspense>
                )}
            </div>
        </div>);
...
async function Karta({numer, realizacja}: { numer: number, realizacja: Realizacja }) {
    const pierwszeZdjecie = await getPierwszyPlik('/realizacje/' + realizacja.typ + '/' + numer);
    const zdjecieURL = await getImage(pierwszeZdjecie.fullPath);

    return (
        <div className={'flex basis-[300px] shrink-0 flex-col gap-5'} >
            <Link href={`/realizacje/${realizacja.typ}/${numer}`} className={'hover:cursor-pointer overflow-hidden h-[400px] relative border-8 border-white rounded-sm'}>
                <Image src={zdjecieURL} alt={'zdjecie'} fill={true} objectFit={'cover'}/>
            </Link>
            <div className={'flex justify-center'}>
                <Link href={`/realizacje/${realizacja.typ}/${numer}`}
                      className={'border-2 border-orangeMain rounded-md py-2 px-10 font-SourGummy text-4xl bg-white'}>
                    <span>{realizacja.typ2}: </span>
                    <span>{numer}</span>
                </Link>
            </div>
        </div>
...
function KontenerKartLoadin({realizacjeData} : {realizacjeData : RealizacjeArray}) {
    return (
        <div className={'flex flex-col'}>
            <div className={`grid grid-cols-fill-300  lg:grid-cols-fill-300 gap-x-5 gap-y-16 justify-around`}>
                {realizacjeData.map((value) => <KartaLoading key={value.id}/>)}
            </div>
        </div>
    );
}